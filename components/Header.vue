	...
	<header>
		<div class="container mx-auto flex items-center justify-between">
			<div class="flex items-center">
				...
				<div class="hidden lg:block text-sm text-gray-500 font-semibold border-l border-gray-100 border-opacity-40  ml-6 pl-6 py-0.5">
					<nav class="w-auto grid grid-flow-col items-center gap-4 md:gap-6 mt-0">
						...
						<NuxtLink to="/more" class="hidden md:inline-flex"
							><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" display="block" id="MoreHorizontal">
								<circle cx="4" cy="12" r="1" />
								<circle cx="12" cy="12" r="1" />
								<circle cx="20" cy="12" r="1" /></svg
						></NuxtLink>
					</nav>
				</div>
			</div>
			<nav class="w-auto grid grid-flow-col items-center gap-3 md:gap-3 mt-0">
				<NuxtLink to="/my-account" class="hidden md:inline-flex items-center justify-center h-9 px-4 font-semibold border text-gray-500 border-gray-100 border-opacity-50 hover:border-opacity-100 hover:text-gray-800 hover:shadow-sm transition text-nav rounded-full focus:shadow-outline focus:outline-none">My Account</NuxtLink>
				...
				<button href="#" class="inline-flex md:hidden text-gray-500" aria-label="Open Menu" title="Open Menu" @click="isMenuOpen = true">
...
nav {
	& > a {
		...
		&.button {
			@apply items-center justify-center h-9 px-4 font-bold text-white bg-gray-900 hover:bg-cc-main hover:text-white transition text-xs border-2 border-transparent rounded-full focus:outline-none;
		}
	}
}
</style>
